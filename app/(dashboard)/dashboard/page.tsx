...
export default async function DashboardPage() {
  ...
  const todaySeconds = sessions?.filter(s => s.started_at.startsWith(today)).reduce((a, s) => a + s.duration_seconds, 0) ?? 0;
  const weekSeconds  = sessions?.reduce((a, s) => a + s.duration_seconds, 0) ?? 0;
  const totalChapters    = subjects?.flatMap(s => (s as any).chapters).length ?? 0;
  const completedChapters = subjects?.flatMap(s => (s as any).chapters).filter((c: any) => c?.is_completed).length ?? 0;
  ...
  return (
    <div className="animate-in">
      <Header title={`Good ${new Date().getHours() < 12 ? 'morning' : new Date().getHours() < 17 ? 'afternoon' : 'evening'}, ${firstName} 👋`} subtitle="Here's your study overview" />
              ...
              <div className="space-y-3">
                {todayPlan.map((plan: any) => (
                  <div key={plan.id} className="flex items-center gap-4 p-4 rounded-xl border" style={{ borderColor: 'var(--border)', background: 'var(--bg)' }}>
                    ...
                    <span className="text-xs font-medium px-3 py-1.5 rounded-lg" style={{ background: 'rgba(59,130,246,0.1)', color: '#3b82f6' }}>
